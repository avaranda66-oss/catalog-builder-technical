...
describe('useCatalogStore', () => {
  ...
  it('deve registrar override local na tabela do catálogo sem mutar a estrutura global', () => {
    ...
    const tableBlock = catalog.pages[1].blocks!.find((b) => b.type === 'custom_table' || b.type === 'table')!;
    ...
    const updatedTable = updatedCatalog.pages[1].blocks!.find((b) => b.type === 'custom_table' || b.type === 'table')!;
  ...
  it('deve restaurar o valor oficial removendo a chave de override local', () => {
    ...
    // Aplica override
    store.updateCellOverride(tableBlock.id, firstRow.id, 'range', '-25 a 140 °C (Personalizado)');
    const page1Table = useCatalogStore.getState().currentCatalog!.pages[1].blocks.find((b) => b.type === 'custom_table' || b.type === 'table')!;
    ...
    const restoredTable = useCatalogStore.getState().currentCatalog!.pages[1].blocks.find((b) => b.type === 'custom_table' || b.type === 'table')!;
