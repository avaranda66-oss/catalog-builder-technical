...
import {
  createW4F1Document,
  W4F1_CATALOG_ID,
  W4F1_MERGED_OBJECT_ID,
  W4F1_OBJECT_ID,
  W4F1_OTHER_CATALOG_ID,
} from './w4f1-table-document';

const INITIAL_MUTATION_ID = 'f1111111-aaaa-4aaa-8aaa-111111111111';
const USER_ID = '99999999-9999-4999-8999-999999999999';

function envelope(document: ReturnType<typeof createW4F1Document>, revision: number, mutationId: string): CatalogPersistenceEnvelope {
  return {
...
const primary = createW4F1Document();
const other = createW4F1Document(W4F1_OTHER_CATALOG_ID, 'Outro catálogo W4.F.1');
...
const runtime = new VNextPersistenceRuntime({
  ...
  authorityScopeId: 'proof:w4f1',
  binding: envelope(primary, 1, INITIAL_MUTATION_ID),
});

function tableState(document: ReturnType<typeof createW4F1Document>, objectId: string) {
  ...
  interface Window {
    __W4F1_PROOF__: {
      state(): {
        ...
        canUndo: boolean;
        canRedo: boolean;
        document: ReturnType<typeof createW4F1Document>;
...
window.__W4F1_PROOF__ = {
  primaryId: W4F1_CATALOG_ID,
  otherId: W4F1_OTHER_CATALOG_ID,
  mainObjectId: W4F1_OBJECT_ID,
  mergedObjectId: W4F1_MERGED_OBJECT_ID,
  state: () => {
    ...
    return {
      ...
      main: tableState(current.document, W4F1_OBJECT_ID),
      merged: tableState(current.document, W4F1_MERGED_OBJECT_ID),
    };
  },
...
if (!root) throw new Error('Missing W4.F.1 proof root');
