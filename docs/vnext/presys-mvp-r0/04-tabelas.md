...
Paint horizontal é downstream da row geometry e jamais mede/muta rows. O renderer trabalha em duas fases do **mesmo componente/árvore**, não em dois engines: (1) transform-free measurement grid com column `trackQ`, padding Q, sem paint, mede conteúdo após fonts/assets/images ready; todo browser rect normaliza primeiro para Q e cada célula mantém `requiredQ = intrinsicContentHeightQ + paddingTopQ + paddingBottomQ`; (2) aplicar AUTO/MIN_MM/FIXED_MM e o solver global R0.1.4, cujas variáveis de row continuam em U mas cujas constraints de conteúdo permanecem em Q até o fit projetado. Com resolved row heights `rowHeightU`, formar boundaries cumulativas `boundaryU[0]=0`, `boundaryU[i+1]=boundaryU[i]+rowHeightU[i]`; projetar cada boundary por U -> Q e definir `rowQ[i]=boundaryQ[i+1]-boundaryQ[i]`. Todo `rowQ > 0`.
...
`RowHeightPolicy` também é FOUNDATION REQUIRED — FROZEN FOR FOUNDATION-PROOF-01. Em R0.1.4, o authored/base U é `AUTO = 0`, `MIN_MM = minMm`, `FIXED_MM = heightMm`; conteúdo Chromium não é incorporado ao base por `qToU`. Toda célula, inclusive `rowSpan=1`, produz uma constraint Q sobre o span. `FIXED_MM` permanece autoral exato. Se o span renderizado não comportar o conteúdo em Q, emitir `ROW_CONTENT_OVERFLOW / ERROR`. Não reduzir fonte, aumentar FIXED, cortar silenciosamente nem alterar conteúdo.
...
Para rows, defina `basePrefixU[k]` como a soma dos bases U até a boundary `k`. Enumere somente rows growable (`AUTO`/`MIN_MM`) e defina `E[j]` como o extra U cumulativo atribuído às primeiras `j` growable rows. Se `g(k)` é a quantidade de growable rows antes da boundary `k`, a boundary física final é `totalBoundaryU[k] = basePrefixU[k] + E[g(k)]`.

Cada célula cobre `[start,end)` e mantém `requiredQ = intrinsicContentHeightQ + paddingTopQ + paddingBottomQ`. O fit obrigatório é `uToQ(totalBoundaryU[end]) - uToQ(totalBoundaryU[start]) >= requiredQ`. Para um span com pelo menos uma growable row, seja `a=g(start)` e `b=g(end)`, com `a < b`. Depois que `E[a]` está fixado, o menor end prefix admissível é `minimumUForProjectedQ(uToQ(basePrefixU[start] + E[a]) + requiredQ)`. Logo a edge impõe `E[b] >= minimumUForProjectedQ(uToQ(basePrefixU[start] + E[a]) + requiredQ) - basePrefixU[end]`. A cadeia também impõe `E[j+1] >= E[j]`.

`minimumUForProjectedQ(targetQ)` é a menor U inteira não-negativa cuja projeção alcança o target Q. Para `targetQ > 0`, a forma matemática é `ceil(((2*targetQ - 1) * 15_875) / 768)` sob a regra round-half-away-from-zero; a implementação deve preservar safe-integer arithmetic e verificar `uToQ(U) >= targetQ` e, para `U>0`, `uToQ(U-1) < targetQ`.

Todas as edges apontam do prefixo growable menor para o maior. Em ordem semântica, calcular `E[b]` como o máximo entre `E[b-1]` e todas as lower bounds das edges que terminam em `b`; como cada lower bound depende somente de `E[a]` já final e `uToQ`/a inversa são monotônicas, indução mostra que qualquer solução factível domina o prefixo construído componente a componente. Portanto o resultado minimiza exatamente o extra U total, é determinístico e independe da ordem das constraints. Extras são derivados por `x[j]=E[j+1]-E[j]` e só existem em AUTO/MIN; FIXED nunca recebe crescimento.

Span sem growable row não cria crescimento fora de seu intervalo para “phase-tuning”. Sua capacidade é a diferença real das boundaries Q finais. Se ficar abaixo de `requiredQ`, emitir `ROW_CONTENT_OVERFLOW / ERROR`. Após projetar todas as rows, o preflight revalida **toda** célula: `availableContentQ = boundaryQ[end]-boundaryQ[start]-paddingTopQ-paddingBottomQ`; `intrinsicContentHeightQ > availableContentQ` bloqueia READY. Em span com growable row, tal violação remanescente é falha do solver e também bloqueia.

**HISTÓRICO R0.1.2/R0.1.3 PRESERVADO:** R0.1.3 eliminou a expansão sequencial artificial e provou o witness U `507789 U` sob a então vigente conversão nearest-Q→U; R0.1.4 reabre somente essa semântica de requisito medido. O counterexample continua obrigatório como regressão global, agora usando o `requiredQ` Chromium original e exigindo mínimo projetado sem reintroduzir a expansão R0.1.2.
