...
`resolveColumns(table, availableInnerWidthMm)` é função pura e retorna `{ok:true,widths}` ou `{ok:false,code,details}`, sem DOM. Invariantes: width > 0; minWidth respeitado; fixed respeitado; flex usa somente espaço restante; deterministic rounding; conservação de largura. Contraexemplo obrigatório: available 100 mm, fixed A 70, fixed B 50, flex C min 20 -> ERROR; jamais valid+warning/C=0.

`validateTable` verifica stable table/row/column/cell IDs, collision-safe cell key, spans, `coveredBy`, merge fail-closed, `MERGE_WOULD_DISCARD_CONTENT` e annotation refs. `TableAnnotation {id, kind:'note'|'footnote'|'caption', text}` possui stable ID; cells/table podem referenciar IDs; dangling reference é ERROR; annotation participa da geometria e aparece na página correta.
...
5. Executar readiness na ordem: render → required fonts loaded → required assets resolved → images decoded successfully → measurement → layout report/preflight → stability check. `setTimeout` não prova readiness.
...
7. Inspecionar conteúdo do PDF com `pdfjs-dist` já disponível quando adequado, validar dimensões/páginas/text layer/códigos/unidades/símbolos e confirmar que página/tabela não virou raster integral. Texto extraível sozinho não basta.
...
| T-STABILITY-01 | Mudança inesperada de geometria após resources-ready/preflight gera `LAYOUT_UNSTABLE / ERROR` |
