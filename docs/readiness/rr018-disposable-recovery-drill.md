...
O TOC do dump é tratado de forma explícita e fail-closed. Como toda Stack B Supabase fresh já contém o schema `public`, o runner comenta exatamente uma entrada TOC `SCHEMA - public ...` para não tentar recriá-lo. Também comenta somente as três entradas de `DEFAULT ACL` pertencentes a `supabase_admin` no schema `public`: `DEFAULT PRIVILEGES FOR TABLES`, `DEFAULT PRIVILEGES FOR FUNCTIONS` e `DEFAULT PRIVILEGES FOR SEQUENCES`. Esses quatro itens são da plataforma Supabase base, não objetos da aplicação. As três entradas `DEFAULT ACL` equivalentes pertencentes a `postgres` permanecem habilitadas na restore list.

Antes de destruir a Stack A, o runner captura proprietário/ACL do schema `public` e as seis DEFAULT ACLs de plataforma para `postgres` e `supabase_admin` nas classes TABLES/FUNCTIONS/SEQUENCES. O TOC e cada snapshot de `pg_default_acl` devem conter exatamente essas seis classes; qualquer sétima entrada, classe ausente ou classe substituta bloqueia o drill para revisão. Depois de iniciar a Stack B fresh, ele exige igualdade byte a byte desses baselines. Como as DEFAULT ACLs de `postgres` já existem na Stack B e concedem privilégios amplos a novos objetos, o runner revoga temporariamente apenas os grants default de `anon`, `authenticated` e `service_role` durante a criação dos objetos do schema. As três entradas `DEFAULT ACL ... postgres` preservadas no próprio dump recompõem a baseline ao final do schema restore, e uma comparação byte a byte é feita imediatamente após o restore e novamente ao fim do drill. Isso evita privilégios residuais de criação sem remover nenhuma ACL explícita da aplicação.
