import type { AssetRef, TableModel } from './proof-model';
import type { TablePlan } from './proof-render-plan';
import { annotationDisplayText,cellDisplayText,legendDisplayText,referencedAnnotations } from './proof-render-plan';
import { orderedAnchors } from './proof-table';
import { mmToU,qCss,uToQ } from './physical';
import { resolveWrap } from './proof-style';
import { ProofRichText,typography } from './ProofRichText';

export function ProofTable({table,plan,assets,assetUrls}:{table:TableModel;plan:TablePlan|undefined;assets:readonly AssetRef[];assetUrls:ReadonlyMap<string,string>}) {
  ...
  const renderAnnotation=(annotation:typeof annotations[number])=><div key={annotation.id} data-annotation-id={annotation.id}
    className={'proof-annotation '+annotation.kind} style={{...typography(plan.annotationStyle),marginBottom:annotation.kind==='caption'?gap:0,marginTop:annotation.kind==='caption'?0:gap}}>
    <div data-flow-root=""><ProofRichText rich={annotationDisplayText(annotation,table)}/></div>
  </div>;
  ...
  return <div data-table-intrinsic={table.id} className="table-intrinsic">
    ...
    <div data-table-id={table.id} role="table" aria-label={table.id} aria-rowcount={table.rows.length} aria-colcount={table.columns.length}
      className="proof-grid" style={{width:qCss(plan.frameQ),gridTemplateColumns:plan.trackQ.map(qCss).join(' '),
      ...
      {table.rows.map((row,r)=><div key={row.id} role="row" aria-rowindex={r+1} className="logical-row">
        {anchors.filter(cell=>cell.rowId===row.id).map(cell=>{
          ...
          return <div key={cell.id} data-cell-id={cell.id} role={row.role==='header'?'columnheader':'cell'}
            ...
            className="proof-cell" style={{...typography(style),background:style.background,
            ...
            <div data-flow-root="">
              {rich&&<ProofRichText rich={rich} nowrap={resolveWrap(cell)==='nowrap'}/>}
    ...
    {annotations.filter(a=>a.kind!=='caption').map(renderAnnotation)}
    {table.legend.length>0&&<div className="proof-legend" style={{...typography(plan.annotationStyle),marginTop:gap}}>
      {table.legend.map(legend=><div key={legend.id} data-annotation-id={legend.id} className="legend-entry">
        <div data-flow-root=""><ProofRichText rich={legendDisplayText(legend)}/></div>
      </div>)}
    </div>}
  </div>;
}
