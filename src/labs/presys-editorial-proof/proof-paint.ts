import type { Cell, TableModel } from './proof-model';
import { cellIndex, getCellKey } from './proof-table';
import { cumulative } from './proof-layout';
import { compareDecimal, ptToQ, sum } from './physical';
import { diagnostic, ProofError, type Diagnostic } from './diagnostics';
import type { EdgeCandidate, ResolvedStyle, Side } from './proof-style';
...
function solid(candidate:EdgeCandidate|undefined):SolidCandidate|undefined {
  ...
  if(candidate.border.pattern!=='solid')throw new ProofError('UNSUPPORTED_BORDER_PATTERN');
...
export function buildPaint(table:TableModel,trackQ:readonly number[],rowQ:readonly number[],styles:ReadonlyMap<string,ResolvedStyle>):{edges:PaintEdge[];diagnostics:Diagnostic[];suppressed:string[]} {
  ...
  const style=(cell:Cell):ResolvedStyle=>{const s=styles.get(cell.id);if(!s)throw new ProofError('STYLE_UNRESOLVED');return s;};
  ...
  function emit(id:string,orientation:'horizontal'|'vertical',r:number,c:number,leading:Cell|undefined,trailing:Cell|undefined):void {
    ...
    if(t>trackQ[ownerCol]||t>rowQ[ownerRow]||t>frameQ||t>heightQ)throw new ProofError('BORDER_PAINT_GEOMETRY_INVALID',id);
    ...
    if(edge.xQ<0||edge.yQ<0||edge.xQ+edge.widthQ>frameQ||edge.yQ+edge.heightQ>heightQ)throw new ProofError('BORDER_PAINT_GEOMETRY_INVALID',id);
