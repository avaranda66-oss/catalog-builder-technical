// src/domain/bottom-header.appearance.ts
// Contrato puro de domínio para paletas visuais e gradientes do Rodapé Técnico / Bottom Header (CORE.E6A).
// Zero dependências de React, Zustand ou Supabase.

export type BottomHeaderPaletteId =
  | 'presys_navy_solid'
  | 'presys_industrial'
  | 'navy_corporate'
  | 'obsidian'
...
export type BottomHeaderForegroundTone = 'light' | 'dark';

export interface BottomHeaderPaletteDefinition {
  readonly id: BottomHeaderPaletteId;
  readonly label: string;
  readonly className: string;
  readonly hex: string;
  readonly foregroundTone: BottomHeaderForegroundTone;
}

export const BOTTOM_HEADER_PALETTES: readonly BottomHeaderPaletteDefinition[] = [
  {
...
export const DEFAULT_BOTTOM_HEADER_PALETTE_ID: BottomHeaderPaletteId = 'presys_navy_solid';

export const BOTTOM_HEADER_PALETTE_OPTIONS = BOTTOM_HEADER_PALETTES.map((p) => ({
  label: p.label,
  value: p.id
}));

export interface BottomHeaderAppearanceTarget {
  style?: {
...
/**
 * Resolve a classe Tailwind para renderização do Bottom Header seguindo:
 ...
 */
export function resolveBottomHeaderPaletteClass(block: BottomHeaderAppearanceTarget | null | undefined): string {
...
/**
 * Identifica o BottomHeaderPaletteId semântico ativo. Se o valor for um gradiente legado
 * fora do allowlist canônico, retorna 'legacy_custom'.
 */
export function resolveBottomHeaderPaletteId(block: BottomHeaderAppearanceTarget | null | undefined): BottomHeaderPaletteId | 'legacy_custom' {
  const currentClass = resolveBottomHeaderPaletteClass(block);
  const found = BOTTOM_HEADER_PALETTES.find((p) => p.className === currentClass);
  if (found) {
    return found.id;
  }
...
/**
 * Resolve o tom de contraste/foreground do Bottom Header:
 ...
 */
export function resolveBottomHeaderForegroundTone(block: BottomHeaderAppearanceTarget | null | undefined): BottomHeaderForegroundTone {
  const paletteId = resolveBottomHeaderPaletteId(block);
  if (paletteId !== 'legacy_custom') {
    const palette = BOTTOM_HEADER_PALETTES.find((p) => p.id === paletteId);
    if (palette) {
...
export interface BottomHeaderPalettePatch {
  style: Record<string, unknown>;
  customData?: Record<string, unknown>;
}

/**
 * Constrói o patch canônico para aplicação de uma paleta no Bottom Header:
 ...
 */
export function setBottomHeaderPalette(
  block: BottomHeaderAppearanceTarget | null | undefined,
  paletteId: BottomHeaderPaletteId
): BottomHeaderPalettePatch {
  const palette = BOTTOM_HEADER_PALETTES.find((p) => p.id === paletteId);
